...
| `gpg.decrypt` | Done | OpenPGP.js decrypt → plaintext, from the pipe or Inputs → OpenPGP (`count=all` for a bundle; `pasted=merge` for both roads; `shares` collects mnemonics) |

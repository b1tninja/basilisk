...
input | utf8 | rsa-oaep key=@rk label=ctx | hex | out @ct
in @ct | hex -d | rsa-oaep -d key=@rk label=ctx | utf8 | out @plain`
    );
    expect(validation.ok).toBe(true);
    const arts = await runRecipe(ast, {
  ...
  it("fails when label mismatches", async () => {
    const { ast } = compileRecipe(
      `genkey rsa/2048 usage=encrypt | out @rk
input | utf8 | rsa-oaep key=@rk label=a | hex | out @ct
in @ct | hex -d | rsa-oaep -d key=@rk label=b | utf8`
    );
    await expect(

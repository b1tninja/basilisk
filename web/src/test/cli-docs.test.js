...
// Normalize CRLF: git checks this file out with native line endings, so on
// Windows every `\n` in the patterns below would otherwise be `\r\n` and the
// fenced-block regexes would silently match nothing. The file reads fine —
// only the assertions were platform-dependent.
const docText = readFileSync(DOC, "utf8").replace(/\r\n/g, "\n");

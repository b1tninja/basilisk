    ...
    >
      ...
      <Sheet open={expanded != null} onOpenChange={(o) => !o && setExpanded(null)}>
        <SheetContent side="right" className="w-[min(560px,100vw)] sm:max-w-none">
          ...
          {expandedRow ? (
            /* Expand is the affordance that gives an artifact room, and for
               one kind it gave none: a QR's intrinsic SVG size won here
               exactly as it did in the row, so a 103px code opened into a
               560px window at 103px. The descendant selector is the whole fix
               — a real stylesheet rule, generated by Tailwind, so nothing is
               written inline (`style-src 'self'`) — and it is expressed here
               rather than as a prop because "this widget is in the Sheet" is
               a fact about the Sheet, not about the QR. */
            <div className="overflow-y-auto px-4 pb-4 [&_[data-qr-artifact]]:w-full">

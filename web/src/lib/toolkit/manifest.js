 ...
 *   pinned clock would flow into, and nothing flows into it. What *is* checked,
 *   before the run rather than after it, is the direction that matters: the
 *   compiler refuses a pooled value becoming key material (see `recipe.js`),
 *   which is a fact about the recipe and needs no manifest at all.
 ...
 *
 * The value exists: `lib/toolkit/entropy-pool.js` is the commit-then-reveal
 * that produces `entropy.digest`, domain-separated in the same family as
 * `PEERS_DOMAIN` and `AUDIENCE_DOMAIN` so a pool digest can never be some other
 * digest of the same bytes. `mode: "pool"` is a *record* of a pool this run
 * drew, never a claim about a notebook that mentions the op.
 *
 * ## What guards it, and what used to claim to
 *
 * The danger is a pooled value becoming key material: if everyone can recompute
 * the key, nobody has a private key. That is checked **by value flow**, in the
 * compiler — a value carrying `pooled` may not reach a step that produces key
 * material, and a param says `acceptsPooled` or does not take one. See
 * `recipe.js`.
 *
 * This module used to carry a whole-notebook check, `mirroredRunRefusals`,
 * which refused a `pool` manifest containing any op that *draws* keying
 * randomness. It was removed, and three things were wrong with it:
 *
 * - **Its premise is not in this build.** It read "seeding this step would give
 *   every peer the same secret", and nothing seeds an op's randomness from a
 *   pool — `genkey` draws from the platform CSPRNG whether or not a pool was
 *   drawn, so every peer gets a different key and the refusal was false.
 * - **It would have refused every manifest this build produces.** A manifest
 *   only says `pool` when a pool was drawn, which needs a live room, which
 *   needs `quorum.join` — and that correctly declares `keying`, for the
 *   ephemeral ECDH the channel is built on.
 * - **It never ran.** No caller ever passed a manifest to `planRun`.
 *
 * The day an op does seed from a pool, the check belongs at the seeding site,
 * where it can ask what is actually being seeded — not over a whole notebook,
 * where it cannot tell a key from a channel.
 *
 * `pool` therefore remains a declared non-reproducible dependency in
 * `manifestReproducibility`: a run that says `pool` still draws its keys
 * locally, which is the honest answer rather than a flattering one.
 ...
 * never "what will the engine do". Whether the recipe contradicts its own
 * declaration is a different question, asked against the recipe: the compiler's
 * pooled-value rule in `recipe.js` is the one that is actually asked today.
 *
 * @param {RunManifest} manifest
...
export function manifestReproducibility(manifest) {
  ...
  if (mode === "pool") {
    reasons.push(
      `entropy: pool ${short(entropy.digest)} — a value the room agreed on, ` +
        "but no op reads a pool yet, so this run still drew its own randomness"
    );
  } else if (mode === "local") {

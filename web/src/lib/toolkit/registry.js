...
export const STEPS = [
  ...
  },
  {
    ...
    shelf: "sign",
    conjugateOf: "sign",
    doc: "Verify a signature over pipeline message bytes. Prefer `verify key=$pub`; else key panel. Default fail-loud; `-q` / `soft=true` emits `true`/`false` instead of throwing on bad sig. Signature via `signature=` or runtime binding. Same `saltLength=` / `hash=` as sign.",
    input: "bytes",
    output: "bool",
    entropy: "none",
    params: [
  ...
  },
  {
    ...
    shelf: "control",
    flowControl: true,
    doc: "Map a required body over a shares collection. Indent `-` lines or `{ … }`. Optional `foreach :items` / `:values` / `:keys`. Tip is a `bundle` of per-item tips (side effects via `out` / auto-emit) — do not pipe the bundle into cipher/KDF ops; use `$slot`s from the body. A body `out $x` emits one tile per iteration and binds `$x` once, to a bundle of every iteration's value — so `$x | shares | …` recovers the set it emitted. Example: `… | blip39 | foreach` / `- out $share` or `- gpg.encrypt`.",
    input: "shares",
    output: "bundle",
    entropy: "none",
    params: [],
  ...
  },
  {
    ...
    toolbox: "sss",
    shelf: "split",
    doc: "Select from a shares collection (1-based). Same as `[1]` / `[1:2]`. When the split's share count is stated in the text (`sss.split 2/3` stamps it on the type), an index past it refuses at compile time — `at 5` of a 3-share split selects nothing. A selected share is an ordinary value: `… | blip39 | [1] | out $share-1` puts share 1 in a slot the next cell can read with `in $share-1`.",
    input: "shares",
    output: "shares",
    entropy: "none",
    params: [
  ...
  },
  {
    ...
    toolbox: "flow",
    shelf: "control",
    doc: "Project a member of the pipeline value. The keypair halves are written as verbs — `public` / `private` — and turn a keypair tip into a `key` tip (CryptoKey half): `genkey ec/p256 | public | export spki | pem`. The colon spelling still reads and canonicalizes to the verb. Item projections keep their colon (`:key` / `:value`), because they name a member of the item a `foreach :items` loop is holding rather than of a pipeline value.",
    input: "bytes",
    output: "bytes",
    entropy: "none",
    params: [
  ...
  },
  {
    ...
    doc: "Verify an OpenPGP cleartext or detached signature. Prefer `gpg.verify key=$pub`. Detached: `signature=$slot`. Fail-loud by default; `-q` / `soft=true` → bool true|false. Distinct from WebCrypto `verify`.",
    input: "text",
    output: "bool",
    entropy: "none",
    params: [
  ...
  },
  {
    ...
    shelf: "ports",
    conjugateOf: "input",
    doc: "Emit a file tile, register a live `$slot` for later `in`, and pass through — always, whatever the value carries (a selected share included). Inside a `foreach` body the slot holds a bundle of every iteration's value. Prefer `out $public` (bare `out public` rewrites to `$`). File paths reserved — not supported yet.",
    input: "text",
    output: "text",
    entropy: "none",
    params: [
  ...
  },
  {
    ...
    toolbox: "io",
    shelf: "ports",
    doc: "Send the value the preceding `out` just named to the room. Writes nothing and computes nothing — it is the one place a recipe says a value may leave the machine that made it, and `planRun` reads it to decide what a handoff may carry. Needs a `@peer` header, because leaving is a claim about a boundary. Example: `random 32 | digest | encode hex | out $expected | publish`.",
    input: "text",
    output: "text",
    entropy: "none",
    params: [],
